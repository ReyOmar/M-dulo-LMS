...
export class MailService implements OnModuleInit {
  ...
  private readonly SAFE_TEMPLATE_KEYS = new Set([
    'url_campus', 'url_curso', 'url_certificado', 'url_solicitudes',
    'url_calificaciones', 'url_monitoreo', 'url_mensajes', 'url_accion',
    'resetUrl',
    'mensaje_aprobacion', 'progreso_cursos', // Pre-formatted HTML blocks generated by server
  ]);
  ...
  async sendInactivityReminder(
    ...
    nombre: string,
    diasInactivo: number,
    progreso?: { curso: string; porcentaje: number }[],
  ) {
    // Build progress summary HTML if available
    let progresoHtml = '';
    if (progreso && progreso.length > 0) {
      const rows = progreso
        .map(
          (p) =>
            `<tr><td style="padding:8px 12px;border-bottom:1px solid #e2e8f0;font-size:13px;color:#334155">${this.escapeHtml(p.curso)}</td><td style="padding:8px 12px;border-bottom:1px solid #e2e8f0;font-size:13px;font-weight:700;color:${p.porcentaje >= 80 ? '#10b981' : p.porcentaje >= 40 ? '#f59e0b' : '#ef4444'};text-align:center">${p.porcentaje}%</td></tr>`,
        )
        .join('');
      progresoHtml = `<table style="width:100%;border-collapse:collapse;margin:16px 0;border:1px solid #e2e8f0;border-radius:8px;overflow:hidden"><thead><tr style="background:#f8fafc"><th style="padding:10px 12px;text-align:left;font-size:11px;text-transform:uppercase;letter-spacing:1px;color:#64748b">Curso</th><th style="padding:10px 12px;text-align:center;font-size:11px;text-transform:uppercase;letter-spacing:1px;color:#64748b">Progreso</th></tr></thead><tbody>${rows}</tbody></table>`;
    }

    const rendered = await this.renderTemplate('RECORDATORIO_INACTIVIDAD', {
      nombre,
      diasInactivo,
      progreso_cursos: progresoHtml,
      url_campus: `${this.appUrl}/dashboard`,
    });
    if (!rendered) return false;
    return this.sendMail(email, rendered.asunto, rendered.html);
  ...
  async sendChatNotification(
    ...
    const rendered = await this.renderTemplate('MENSAJE_CHAT', {
      nombre,
      remitente,
      mensaje_preview: mensaje.length > 200 ? mensaje.substring(0, 200) + '...' : mensaje,
      origen,
      url_mensajes: `${this.appUrl}/dashboard/mensajes`,
    });
    if (!rendered) return false;
    return this.sendMail(email, rendered.asunto, rendered.html);
  }
   ...
   */
  async sendCertificateGenerated(email: string, nombre: string, cursoTitulo: string, codigo: string, _certGuid: string) {
  ...
  async sendSubmissionRejected(
    ...
    // SEC: Escape user-provided comentario to prevent XSS via email
    const comentarioHtml = comentario
      ? `<div style="background:#f1f5f9;border-radius:12px;padding:16px;margin:16px 0"><p style="color:#64748b;font-size:13px;margin:0"><strong>Comentario del examinador:</strong> ${this.escapeHtml(comentario)}</p></div>`
      : '';
