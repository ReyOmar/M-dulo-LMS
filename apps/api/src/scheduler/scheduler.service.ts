    ...
    return this.withJobLock('checkInactiveStudents', async () => {
      ...
      const inactiveStudents = await this.prisma.usuarios.findMany({
        where: {
          rol: 'ESTUDIANTE',
          activo: true,
          OR: [{ ultimo_acceso: { lt: threeDaysAgo } }, { ultimo_acceso: null }],
        },
        ...
        if (!recentReminder) {
          ...
          if (matriculas.length > 0) {
            const allResourceGuids = matriculas.flatMap((m) =>
              m.curso.modulos.flatMap((mod) => mod.lecciones.flatMap((l) => l.recursos.map((r) => r.guid))),
            );
            ...
            progreso = matriculas.map((m) => {
              ...
              const completados = m.curso.modulos.reduce(
                (sum, mod) =>
                  sum +
                  mod.lecciones.reduce((s, l) => s + l.recursos.filter((r) => completedSet.has(r.guid)).length, 0),
                0,
              );
              return {
          ...
          await this.mailService.sendInactivityReminder(student.email, student.nombre, diasInactivo, progreso);
        ...
        if (!currentDraftSet.has(prevDraftGuid)) {
          ...
          if (curso && curso.estado === 'PUBLICADO') {
            ...
            for (const m of matriculas) {
              ...
              // Email notification
              await this.mailService.sendCourseReactivated(m.usuario.email, m.usuario.nombre, curso.titulo);
            }
      ...
      for (const submission of pendingSubmissions) {
        ...
        if (submission.calificacion != null) {
          ...
          if (Number(submission.calificacion) < notaAprobacion) {
            this.logger.warn(
              `Skipping auto-unlock for submission ${submission.guid}: grade ${submission.calificacion} < ${notaAprobacion}`,
            );
  ...
  async purgeRetroactiveEntregaFiles() {
    return this.withJobLock('purgeRetroactiveEntregaFiles', async () => {
      ...
      try {
        ...
        const entregas = await this.prisma.$queryRaw<
          Array<{
            guid: string;
