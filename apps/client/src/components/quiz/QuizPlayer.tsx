...
export default function QuizPlayer({
  ...
  const resetQuiz = async () => {
    ...
    const success = showResultModal
      ? showResultModal.nota >= 3.0
      : quizStatus?.mejor_nota
        ? quizStatus.mejor_nota >= 3.0
        : false;
    ...
    return (
      <div className="bg-gradient-to-br from-emerald-500/5 to-emerald-950/20 flex flex-col items-center justify-center p-12 text-center border border-emerald-500/30 rounded-[2rem] shadow-sm animate-in fade-in duration-500">
        ...
        <p className="text-muted-foreground mb-8 text-lg max-w-sm">
          ...
          <strong className="text-emerald-500">{quizStatus.mejor_nota.toFixed(1)}</strong>.
        </p>
      ...
      {started && (
        <div className="bg-background/80 backdrop-blur-md border-b border-border/50 p-4 flex items-center justify-between sticky top-0 z-30 animate-in slide-in-from-top duration-300">
          ...
          <div className="flex items-center gap-4">
            {timeLeft !== null && (
              <div
                className={`flex items-center gap-2 px-3 py-1.5 rounded-xl font-black text-xs border transition-colors ${timeLeft < 60 ? 'bg-red-500/20 text-red-500 border-red-500/50 animate-pulse' : 'bg-muted text-foreground border-border'}`}
              >
      ...
      {!started ? (
        <div className="animate-in fade-in duration-500">
          ...
          <div className="p-6 md:p-10 border-b border-border bg-amber-500/10">
            <h3 className="font-bold text-xs text-amber-700 dark:text-amber-500 uppercase tracking-widest mb-4">
            ...
            <div
              ...
              dangerouslySetInnerHTML={sanitizeHTML(
                instrucciones_html ||
                  '<p class="text-muted-foreground italic opacity-70">Sin instrucciones específicas.</p>',
              )}
            />
              ...
              <div className="mt-8 space-y-3">
                {url_referencia && (
                  <a
                    ...
                    className="flex items-center gap-3 p-4 bg-background/50 border border-primary/20 rounded-2xl text-sm text-primary font-bold hover:bg-primary/5 transition-all group"
                  >
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <span className="text-sm font-black text-foreground block truncate">
                        {archivo_adjunto_nombre || 'Descargar archivo adjunto'}
                      </span>
          ...
          <div className="p-10 text-center relative overflow-hidden">
            ...
            <div className="relative z-10 py-4">
              ...
              <h2 className="text-2xl font-black mb-6 tracking-tight text-foreground uppercase tracking-widest">
              ...
              >
                {isStarting && (
                  <span className="w-5 h-5 border-2 border-muted-foreground border-t-transparent rounded-full animate-spin"></span>
                )}
        ...
        <div className="p-6 md:p-12 space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
          {quizConfig.preguntas.map((q, i) => (
            <div
              key={q.id}
              className="bg-background border border-border/60 rounded-[2rem] p-6 md:p-10 shadow-sm hover:shadow-md transition-all relative overflow-hidden group"
            >
              ...
              <h3 className="font-bold text-xl md:text-2xl mb-8 flex gap-5 text-foreground leading-relaxed">
                <span className="flex-shrink-0 w-10 h-10 md:w-12 md:h-12 rounded-2xl bg-primary/10 text-primary flex items-center justify-center text-lg md:text-xl font-black shadow-inner">
                  {i + 1}
                </span>
              ...
              <div className="space-y-3 md:pl-16">
                {q.opciones.map((opt) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`w-6 h-6 md:w-7 md:h-7 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all duration-300 ${isSelected ? 'border-primary bg-primary/10' : 'border-muted-foreground/30'}`}
                      >
                        <div
                          className={`w-3 h-3 md:w-3.5 md:h-3.5 bg-primary rounded-full transition-transform duration-300 ${isSelected ? 'scale-100' : 'scale-0'}`}
                        />
                      </div>
                      <span
                        className={`text-base md:text-lg ${isSelected ? 'font-black text-foreground' : 'text-muted-foreground font-semibold'}`}
                      >
                        {opt.texto}
                      </span>
                    </div>
            ...
            <div className="flex items-center gap-5 mb-8">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground font-bold mt-1">
                  Perderás tu progreso actual y se contará como un intento.
                </p>
              </div>
            </div>
      ...
      {showResultModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/90 backdrop-blur-xl animate-in fade-in duration-500">
          <div className="bg-card border border-border shadow-2xl rounded-[3rem] w-full max-w-sm p-10 text-center animate-in zoom-in-95 duration-500">
            ...
            <p
              className={`font-black text-xl mb-10 uppercase tracking-widest ${showResultModal.nota >= 3.0 ? 'text-emerald-500' : 'text-red-500'}`}
            >
            ...
            <div className="bg-muted/50 rounded-[2rem] p-6 mb-10 text-sm flex justify-around border border-border/50">
              <div className="text-center">
                ...
                <div className="text-muted-foreground text-[10px] font-black uppercase tracking-widest mt-1">
                  Aciertos
                </div>
              </div>
