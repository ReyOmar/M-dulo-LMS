...
const colorMap: Record<string, { bg: string; text: string; glow: string }> = {
  primary: {
    ...
    text: 'text-primary',
    glow: 'group-hover:shadow-[var(--shadow-glow-primary)]',
  },
  accent: {
    bg: 'from-accent/15 to-accent/5',
    text: 'text-accent',
    glow: 'group-hover:shadow-[var(--shadow-glow-accent)]',
  },
  success: {
    bg: 'from-success/15 to-success/5',
    text: 'text-success',
    glow: 'group-hover:shadow-[var(--shadow-glow-success)]',
  },
...
export function StatCard({
  ...
  return (
    ...
    >
      {/* Background glow orb */}
      <div
        className={`absolute -top-8 -right-8 h-28 w-28 rounded-full bg-gradient-to-br ${c.bg} blur-2xl group-hover:scale-150 transition-transform duration-500 pointer-events-none`}
      />
      ...
      <div className="relative z-10 flex justify-between items-start">
        <div>
          ...
          {trend && (
            <p
              className={`text-xs font-semibold mt-2 flex items-center gap-1 ${trend.value >= 0 ? 'text-success' : 'text-destructive'}`}
            >
        ...
        <div
          className={`p-2.5 rounded-xl bg-gradient-to-br ${c.bg} group-hover:scale-110 transition-transform duration-200`}
        >
