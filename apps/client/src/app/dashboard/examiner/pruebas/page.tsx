...
export default function MonitoreoPruebasPage() {
  ...
  const cursosAgrupados = useMemo(() => {
    ...
    cursos.forEach(c => {
      map.set(c.guid, {
        guid: c.guid, titulo: c.titulo, imagen_portada: c.imagen_portada,
        intentos_totales: 0, aprobados: 0, reprobados: 0, suma_notas: 0, promedio: 0, tasa_aprobacion: 0, entregas: [],
    ...
    entregas.forEach(e => {
      if (!map.has(e.curso_guid)) {
        map.set(e.curso_guid, {
          guid: e.curso_guid, titulo: e.curso_titulo, imagen_portada: null,
          intentos_totales: 0, aprobados: 0, reprobados: 0, suma_notas: 0, promedio: 0, tasa_aprobacion: 0, entregas: [],
          estudiantesMap: new Map<string, any>()
        });
      }
  ...
  const coursePieData = selectedCourse && (selectedCourse.aprobados > 0 || selectedCourse.reprobados > 0)
    ? [
    ...
    return Object.entries(groups).map(([modulo_titulo, tareasMap]) => {
      ...
      const tareas = Object.entries(tareasMap).map(([tarea_titulo, estudiantesMap]) => {
        ...
        const estudiantesList = Object.entries(estudiantesMap).map(([studentId, entregasDelEstudiante]) => {
            const entregasOrdenadas = [...entregasDelEstudiante].sort((a, b) => new Date(b.fecha_entrega).getTime() - new Date(a.fecha_entrega).getTime());
  ...
  if (!selectedCourseGuid && !loading) {
        ...
        <div className="animate-in fade-in duration-700">
          ...
          <div className="bg-card border border-dashed border-border rounded-2xl p-16 text-center shadow-sm">
            ...
            <p className="text-muted-foreground text-sm mt-1">
              Actualmente no tienes cursos asignados para analizar.
            </p>
          </div>
        </div>
      );
    ...
    const tasaAprobacionGlobal = totalCalificadosGlobal > 0 ? Math.round((totalAprobadosGlobal / totalCalificadosGlobal) * 100) : 0;

    const globalPieData = totalCalificadosGlobal > 0 
      ? [
      ...
      <div className="animate-in fade-in duration-700">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          <div className="bg-card border border-border/50 rounded-2xl p-5 shadow-sm">
            ...
            <p className="text-xs text-muted-foreground font-medium uppercase tracking-wider mt-1">Estudiantes Calificados</p>
          </div>
          <div className="bg-card border border-border/50 rounded-2xl p-5 shadow-sm">
            <div className="flex items-center gap-3 mb-1">
            ...
            <p className="text-xs text-muted-foreground font-medium uppercase tracking-wider mt-1">Tasa de Aprobación Global</p>
          </div>
          <div className="bg-card border border-border/50 rounded-2xl p-5 shadow-sm relative overflow-hidden">
            ...
            <div className="relative z-10">
                ...
                <p className="text-xs text-muted-foreground font-medium uppercase tracking-wider mt-1">Promedio Global (0-5)</p>
            </div>
          </div>
        </div>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    ...
                    <RechartsTooltip 
                      ...
                      contentStyle={{ backgroundColor: 'hsl(var(--card))', borderColor: 'hsl(var(--border))', borderRadius: '12px', fontWeight: 'bold', fontSize: '12px', color: 'hsl(var(--foreground))' }}
                ...
                {courseBarData.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      ...
                      <XAxis 
                        dataKey="name" 
                        axisLine={false} tickLine={false} tick={{ fill: 'currentColor', fontSize: 11, fontWeight: 600 }}
                        className="text-muted-foreground" dy={10}
                      />
                      <YAxis 
                        domain={[0, 5]} allowDecimals={false} axisLine={false} tickLine={false}
        ...
        {filteredCursos.length === 0 ? (
          <div className="bg-card border border-dashed border-border rounded-2xl p-16 text-center shadow-sm">
            ...
            <p className="text-muted-foreground text-sm mt-1">No se encontraron cursos que coincidan con la búsqueda.</p>
          </div>
        ) : (
                  ...
                  <div className="flex items-center gap-5 mt-auto bg-background/50 p-4 rounded-2xl border border-border/30">
                    ...
                    <div className="flex-1 space-y-1">
                      <div className="flex justify-between text-xs">
                        ...
                        <span className={`font-black text-sm ${isBad ? 'text-red-500' : isExcellent ? 'text-emerald-500' : 'text-primary'}`}>{curso.promedio}</span>
                      </div>
                      <div className="flex justify-between text-xs mt-1 pt-1 border-t border-border/50">
                        <span className="text-muted-foreground">Aprobados: <strong className="text-emerald-500">{curso.aprobados}</strong></span>
                        <span className="text-muted-foreground">Reprobados: <strong className="text-red-500">{curso.reprobados}</strong></span>
                      </div>
                    </div>
                  </div>
                </div>
    ...
    <div className="animate-in slide-in-from-right-4 duration-500">
      ...
      <header className="mb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-card border border-border/50 rounded-2xl p-5 shadow-sm">
            ...
            <p className="text-xs text-muted-foreground font-medium uppercase tracking-wider mt-1">Estudiantes Evaluados</p>
          </div>
          <div className="bg-card border border-border/50 rounded-2xl p-5 shadow-sm">
            <div className="flex items-center gap-3 mb-1">
              <div className="w-9 h-9 rounded-xl bg-emerald-500/10 flex items-center justify-center">
            ...
            <p className="text-xs text-muted-foreground font-medium uppercase tracking-wider mt-1">Tasa de Aprobación</p>
          </div>
          <div className="bg-card border border-border/50 rounded-2xl p-5 shadow-sm">
            <div className="flex items-center gap-3 mb-1">
              <div className="w-9 h-9 rounded-xl bg-primary/10 flex items-center justify-center">
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    ...
                    stroke="var(--card)" strokeWidth={4} cornerRadius={6}
                  >
              ...
              {gradeDistribution.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={gradeDistribution} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="currentColor" className="text-border/40" />
                    <XAxis 
                      dataKey="name" axisLine={false} tickLine={false} tick={{ fill: 'currentColor', fontSize: 11, fontWeight: 600 }}
                      className="text-muted-foreground" dy={10}
                    />
                    <YAxis 
                      allowDecimals={false} axisLine={false} tickLine={false} tick={{ fill: 'currentColor', fontSize: 12 }}
      ...
      {entregasAgrupadas.length === 0 ? (
        <div className="bg-card border border-dashed border-border rounded-3xl p-16 text-center">
          ...
          <p className="text-muted-foreground text-sm mt-1">No se encontraron cuestionarios que coincidan con los filtros.</p>
        </div>
      ) : (
            ...
            <div key={modulo.modulo_titulo} className="animate-in slide-in-from-bottom-2 duration-300">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground font-medium mt-0.5">Promedio del módulo: <strong className="text-primary">{modulo.promedio}</strong></p>
                </div>
                
                <div className="p-2 bg-background/50 rounded-full shrink-0">
                  <ChevronDown className={`h-5 w-5 text-primary transition-transform duration-300 ${isModExpanded ? 'rotate-180' : ''}`} />
                </div>
              </div>
                  ...
                  {modulo.tareas.map(grupo => {
                    ...
                    <div key={grupo.tarea_titulo} className="space-y-4">
                      ...
                      >
                        ...
                        <h3 className="text-base font-bold text-foreground flex-1 truncate group-hover:text-primary transition-colors">{grupo.tarea_titulo}</h3>
                        
                        <div className="flex items-center gap-3">
                          <span className="text-xs font-bold bg-muted px-3 py-1 rounded-full text-muted-foreground">Promedio: {grupo.promedio}</span>
                            ...
                            return (
                              <div
                                ...
                                className={`bg-background border rounded-2xl overflow-hidden shadow-sm transition-all hover:shadow-md ${
                                  isAprobado ? "border-emerald-500/30" : isReprobado ? "border-red-500/30" : "border-border"
                                }`}
                              >
                                <div 
                                    className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 cursor-pointer select-none group/est"
                                    onClick={() => setExpandedStudents(prev => ({ ...prev, [estKey]: !prev[estKey] }))}
                                >
                                  <div className="flex-1 min-w-0 flex items-center gap-3">
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold shrink-0 shadow-inner ${
                                      isAprobado ? "bg-emerald-500/10 text-emerald-600" : isReprobado ? "bg-red-500/10 text-red-600" : "bg-muted text-muted-foreground"
                                    }`}>
                                    ...
                                    <div className="truncate flex-1">
                                        ...
                                        <p className="text-xs text-muted-foreground truncate flex items-center gap-1 mt-0.5">
                                          <History className="h-3 w-3" /> {estItem.cantidadIntentos} intento{estItem.cantidadIntentos !== 1 && 's'} realizado{estItem.cantidadIntentos !== 1 && 's'}
                                        </p>
                                    </div>
                                  </div>
                                  ...
                                  <div className="shrink-0 flex items-center gap-4">
                                    {nota !== null ? (
                                      <div className={`px-4 py-2 rounded-xl flex items-center gap-1 ${isAprobado ? 'bg-emerald-500/10' : 'bg-red-500/10'}`}>
                                        <span className={`text-lg font-black ${isAprobado ? 'text-emerald-600' : 'text-red-600'}`}>{nota.toFixed(1)}</span>
                                        <span className={`text-xs font-bold hidden sm:inline ${isAprobado ? 'text-emerald-600/50' : 'text-red-600/50'}`}>/5.0</span>
                                      </div>
                                    ) : (
                                    ...
                                    <ChevronDown className={`h-5 w-5 text-muted-foreground transition-transform duration-300 ${isEstExpanded ? 'rotate-180' : ''}`} />
                                  </div>
                                </div>
                                            ...
                                            {estItem.intentos.map((intento: any, idx: number) => {
                                                ...
                                                <div key={intento.guid} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group/intento">
                                                    ...
                                                    <div className="w-[calc(100%-3rem)] md:w-[calc(50%-2rem)] p-3 rounded-xl bg-card border border-border/50 shadow-sm flex items-center justify-between hover:border-primary/30 transition-colors">
                                                        ...
                                                        {intentoNota !== null ? (
                                                            <div className={`px-2.5 py-1 rounded-md ${isIntAprobado ? 'bg-emerald-500/10' : 'bg-red-500/10'}`}>
                                                              <span className={`text-sm font-black ${isIntAprobado ? 'text-emerald-600' : 'text-red-600'}`}>{intentoNota.toFixed(1)}</span>
                                                            </div>
                                                        ) : (
