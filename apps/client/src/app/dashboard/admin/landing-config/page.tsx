...
export default function LandingConfigPage() {
  ...
  const inputClass = "w-full bg-muted/50 border border-border rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary/50 text-sm transition-all";
    ...
    <div className="p-6 max-w-4xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Personaliza el contenido de la página principal y los textos legales. Los cambios se reflejan en tiempo real.</p>
        </div>
        <button
          onClick={handleSave}
          disabled={saving}
      ...
      <div className="bg-card/50 backdrop-blur-sm border border-border/50 rounded-2xl p-8 animate-in fade-in duration-300">
        {activeTab === "hero" && (
          <div className="space-y-6">
            ...
            <p className="text-sm text-muted-foreground -mt-4 mb-6">Personaliza los títulos y subtítulo que aparecen al entrar a la landing page.</p>
        ...
        {activeTab === "contacto" && (
          <div className="space-y-6">
            ...
            <p className="text-sm text-muted-foreground -mt-4 mb-6">Datos de contacto que se muestran en la sección &quot;Contáctanos&quot; de la landing page.</p>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className={fieldGroup}>
                <label className={labelClass}>
                  <Phone className="h-3.5 w-3.5 inline mr-1.5 text-primary" />Teléfono
                </label>
                <input type="text" value={telefono} onChange={(e) => setTelefono(e.target.value)} className={inputClass} placeholder="+57 300 123 4567" />
              </div>
              <div className={fieldGroup}>
                <label className={labelClass}>Horario de Atención</label>
                <input type="text" value={telefonoSub} onChange={(e) => setTelefonoSub(e.target.value)} className={inputClass} placeholder="Lun-Vie 8am-6pm" />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className={fieldGroup}>
                <label className={labelClass}>
                  <Mail className="h-3.5 w-3.5 inline mr-1.5 text-primary" />Email
                </label>
                <input type="text" value={email} onChange={(e) => setEmail(e.target.value)} className={inputClass} placeholder="contacto@empresa.com" />
              </div>
              <div className={fieldGroup}>
                <label className={labelClass}>Nota de Email</label>
                <input type="text" value={emailSub} onChange={(e) => setEmailSub(e.target.value)} className={inputClass} placeholder="Respuesta en 24h" />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className={fieldGroup}>
                <label className={labelClass}>
                  <MapPin className="h-3.5 w-3.5 inline mr-1.5 text-primary" />Ciudad / Ubicación
                </label>
                <input type="text" value={oficina} onChange={(e) => setOficina(e.target.value)} className={inputClass} placeholder="Bogotá, Colombia" />
              </div>
              <div className={fieldGroup}>
                <label className={labelClass}>Dirección Completa</label>
                <input type="text" value={oficinaSub} onChange={(e) => setOficinaSub(e.target.value)} className={inputClass} placeholder="Cra 7 #45-21, Oficina 302" />
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === "legal" && (
          <div className="space-y-6">
            ...
            <p className="text-sm text-muted-foreground -mt-4 mb-6">Edita los textos que aparecen en la página <strong>/legal</strong>. Si los dejas vacíos, se usará un texto por defecto.</p>
        ...
        {activeTab === "footer" && (
          <div className="space-y-6">
            ...
            <p className="text-sm text-muted-foreground -mt-4 mb-6">Texto descriptivo que aparece en la parte inferior de la landing page.</p>
            ...
            {/* Preview */}
            <div className="mt-6 p-6 rounded-2xl bg-muted/30 border border-border/30">
              <p className="text-xs font-bold text-muted-foreground mb-3 uppercase tracking-wider">Vista Previa del Footer</p>
