...
import { ArrowLeft, CheckCircle, Clock, AlertTriangle, Send, Trophy, XCircle } from 'lucide-react';
...
export default function StudentQuizPage({ params }: { params: Promise<{ curso_id: string; recurso_guid: string }> }) {
  ...
  const [showResultModal, setShowResultModal] = useState<{ nota: number; correctas: number; total: number } | null>(
    null,
  );
    ...
    const fetchQuizData = async () => {
      ...
      try {
        const [resCurso, resStatus] = await Promise.all([
          ...
          api.get(`/cursos/student/quiz/${resolvedParams.recurso_guid}/status`),
        ]);
  ...
  const handleStart = async () => {
    try {
      const res = await api.post(`/cursos/student/quiz/${resolvedParams.recurso_guid}/start`);
      if (res.data?.success) {
        if (quizConfig && quizConfig.tiempo_minutos > 0) {
          // Use server's fecha_inicio for accurate time calculation
          const serverStart = res.data.fecha_inicio ? new Date(res.data.fecha_inicio).getTime() : Date.now();
          const elapsed = Math.floor((Date.now() - serverStart) / 1000);
          const remaining = Math.max(0, quizConfig.tiempo_minutos * 60 - elapsed);
      ...
      console.error(err);
      showAlert.error('Error', 'No se pudo iniciar el cuestionario. Verifica tu conexión e inténtalo de nuevo.');
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await api.post(`/cursos/student/quiz/${resolvedParams.recurso_guid}/submit`, {
        respuestas,
      });
      setShowResultModal({
        nota: res.data.nota,
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center">
        ...
        <button
          ...
          className="px-6 py-2 bg-primary text-white font-bold rounded-xl mt-4"
        >
          Volver al curso
        </button>
      </div>
      ...
      <div className="min-h-screen bg-gradient-to-br from-background to-emerald-950/20 flex flex-col items-center justify-center p-6 text-center">
        <div className="bg-background/60 backdrop-blur-xl border border-emerald-500/30 p-12 rounded-[2.5rem] shadow-2xl max-w-lg w-full animate-in zoom-in-95 duration-700">
          ...
          <p className="text-muted-foreground mb-6 text-lg">
            Ya superaste este módulo con una nota de{' '}
            <strong className="text-emerald-500">{quizStatus.mejor_nota.toFixed(1)}</strong>. No necesitas volver a
            tomarlo.
          </p>
          <button
            onClick={() => router.push(`/cursos/${resolvedParams.curso_id}`)}
            className="w-full py-4 bg-emerald-500 hover:bg-emerald-600 text-white text-lg font-bold rounded-2xl shadow-lg shadow-emerald-500/30 transition-all hover:-translate-y-1"
          >
            Continuar Curso
          </button>
        </div>
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-background to-red-950/20 flex flex-col items-center justify-center p-6 text-center">
        <div className="bg-background/60 backdrop-blur-xl border border-red-500/30 p-12 rounded-[2.5rem] shadow-2xl max-w-lg w-full animate-in zoom-in-95 duration-700">
          ...
          <p className="text-muted-foreground mb-6 text-lg">
            Has agotado tus <strong>{quizConfig.intentos_permitidos}</strong> intentos permitidos para este cuestionario
            y no lograste la nota mínima aprobatoria.
          </p>
          ...
          <button
            onClick={() => router.push(`/cursos/${resolvedParams.curso_id}`)}
            className="w-full py-4 bg-red-500 hover:bg-red-600 text-white text-lg font-bold rounded-2xl shadow-lg shadow-red-500/30 transition-all hover:-translate-y-1"
          >
            Volver al curso
          </button>
        </div>
  ...
  const progressPercentage =
    quizConfig.preguntas.length > 0 ? (Object.keys(respuestas).length / quizConfig.preguntas.length) * 100 : 0;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-background to-muted/30 pb-32">
      ...
      <header className="sticky top-0 z-40 bg-background/70 backdrop-blur-xl border-b border-border/50 h-16 flex items-center px-4 md:px-8 shadow-sm">
        <button
          onClick={() => router.push(`/cursos/${resolvedParams.curso_id}`)}
          className="p-2 hover:bg-muted rounded-full transition-colors mr-4"
        >
        ...
        {started && timeLeft !== null && (
          <div
            className={`flex items-center gap-2 px-5 py-2 rounded-2xl font-black text-sm border shadow-sm transition-colors ${timeLeft < 60 ? 'bg-red-500/20 text-red-500 border-red-500/50 animate-pulse' : 'bg-background/50 backdrop-blur-md text-foreground border-border'}`}
          >
        ...
        {!started ? (
          <div className="bg-background/50 backdrop-blur-2xl border border-border/60 rounded-[2.5rem] p-8 md:p-14 text-center shadow-2xl max-w-2xl mx-auto animate-in zoom-in-95 duration-700 relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <p className="text-muted-foreground mb-8 text-lg leading-relaxed">
                Este cuestionario consta de{' '}
                <strong className="text-foreground">{quizConfig.preguntas.length} preguntas</strong>.
                {quizConfig.tiempo_minutos > 0
                  ? ` Tienes un límite de ${quizConfig.tiempo_minutos} minutos.`
                  : ' No hay límite de tiempo.'}
              </p>

              <div className="flex justify-center gap-4 mb-10">
                <div className="bg-muted/50 px-6 py-3 rounded-2xl border border-border">
                  <span className="text-xs text-muted-foreground uppercase tracking-widest font-bold block mb-1">
                    Intentos
                  </span>
                  <span className="font-black text-xl">
                    {quizStatus.intentos_realizados} / {quizConfig.intentos_permitidos}
                  </span>
                </div>
                <div className="bg-muted/50 px-6 py-3 rounded-2xl border border-border">
                  <span className="text-xs text-muted-foreground uppercase tracking-widest font-bold block mb-1">
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
            {quizConfig.preguntas.map((q, i) => (
              <div
                key={q.id}
                className="bg-background/80 backdrop-blur-xl border border-border/60 rounded-3xl p-6 md:p-8 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group"
              >
                ...
                <h3 className="font-bold text-xl mb-6 flex gap-4 text-foreground/90 leading-relaxed">
                  <span className="flex-shrink-0 w-10 h-10 rounded-2xl bg-primary/10 text-primary flex items-center justify-center text-lg font-black shadow-inner">
                    {i + 1}
                  </span>
                ...
                <div className="space-y-3 md:pl-14">
                  {q.opciones.map((opt) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`w-6 h-6 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all duration-300 ${isSelected ? 'border-primary bg-primary/10' : 'border-muted-foreground/30'}`}
                        >
                          <div
                            className={`w-3 h-3 bg-primary rounded-full transition-transform duration-300 ${isSelected ? 'scale-100' : 'scale-0'}`}
                          />
                        </div>
                        <span
                          className={`text-base md:text-lg ${isSelected ? 'font-bold text-foreground' : 'text-muted-foreground font-medium'}`}
                        >
                          {opt.texto}
                        </span>
                      </div>
      ...
      {showResultModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/90 backdrop-blur-xl">
          <div className="bg-card border border-border shadow-2xl rounded-[2.5rem] w-full max-w-sm p-10 text-center animate-in zoom-in-95 duration-500">
            <div
              className={`w-28 h-28 rounded-full flex items-center justify-center mx-auto mb-8 shadow-inner ${
                showResultModal.nota >= 3.0
            ...
            <h3 className="font-black text-5xl mb-3 text-foreground tracking-tighter">
              {showResultModal.nota.toFixed(1)}{' '}
              <span className="text-2xl text-muted-foreground font-bold tracking-normal">/ 5.0</span>
            </h3>
            <p
              className={`font-black text-lg mb-8 uppercase tracking-widest ${showResultModal.nota >= 3.0 ? 'text-emerald-500' : 'text-red-500'}`}
            >
            ...
            <div className="bg-muted/50 rounded-2xl p-5 mb-10 text-sm flex justify-around border border-border/50">
              ...
              <div className="text-center">
                <div className="font-black text-2xl text-foreground">
                  {showResultModal.total - showResultModal.correctas}
                </div>
