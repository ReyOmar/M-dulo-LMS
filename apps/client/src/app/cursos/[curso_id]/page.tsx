              ...
              {selectedRecurso.tipo === 'TEXTO' && (
                <div className="space-y-6">
                  ...
                  {selectedRecurso.archivo_adjunto && (
                    <button
                      onClick={() =>
                        secureDownload(selectedRecurso.archivo_adjunto, selectedRecurso.archivo_adjunto_nombre)
                      }
                      className="flex items-center gap-3 p-3 bg-muted/30 border border-border rounded-xl hover:bg-primary/10 transition-colors w-full text-left cursor-pointer"
                    >
                ...
                <div className="space-y-6">
                  {selectedRecurso.contenido_html?.includes('youtube.com/watch?v=') &&
                    (() => {
                      try {
                        const videoId =
                          new URL(selectedRecurso.contenido_html).searchParams
                            .get('v')
                  ...
                  {selectedRecurso.contenido_html?.includes('youtu.be/') &&
                    (() => {
                      try {
                        const videoId = (
                          selectedRecurso.contenido_html.split('youtu.be/')[1]?.split('?')[0] || ''
                        ).replace(/[^a-zA-Z0-9_-]/g, '');
                        return (
                          <div className="w-full rounded-2xl overflow-hidden border border-border shadow-sm">
                            <iframe
