...
import {
  ArrowLeft, Check, FileText, UploadCloud, File as FileIcon,
  Loader2, ChevronRight, Users, Clock, CheckCircle2, Edit3,
  ClipboardList, Download, AlertCircle, Calendar, Award
} from "lucide-react";
...
export default function TareaVisorPage() {
  ...
  useEffect(() => { if (curso_id) fetchCurso(); }, [curso_id]);
  useEffect(() => { if (isTeacher && tarea_id) fetchEntregas(); }, [isTeacher, tarea_id]);
  ...
  if (!tarea) return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-4">
      ...
      <Link href={`/cursos/${curso_id}`} className="text-primary font-bold hover:underline">Volver al curso</Link>
    </div>
  );
    ...
    <div className="min-h-screen bg-background pb-16">
      ...
      <header className="sticky top-0 z-50 bg-background/80 backdrop-blur-md border-b border-border/50 h-16 flex items-center px-6">
        ...
        <div className="flex items-center text-sm font-semibold text-muted-foreground truncate flex-1">
          {curso?.titulo} <ChevronRight className="h-4 w-4 mx-2 flex-shrink-0" /> <span className="text-foreground truncate">{cleanTitle}</span>
        </div>
        {isTeacher && (
          <span className="ml-4 text-xs font-bold px-3 py-1 bg-amber-500/10 text-amber-600 rounded-full whitespace-nowrap">Modo Examinador</span>
        )}
      </header>
          ...
          <>
            ...
            <div className="bg-gradient-to-r from-amber-500/5 to-orange-500/5 border border-amber-500/20 rounded-2xl p-6 mb-8">
              ...
              <div className="flex flex-wrap items-center justify-center gap-3">
                <button
                  onClick={async () => { if (!showEntregas) await fetchEntregas(); setShowEntregas(v => !v); }}
                ...
                >
                  {loadingEntregas ? <><Loader2 className="h-5 w-5 animate-spin" /> Cargando...</>
                    : isQuiz ? <><ClipboardList className="h-5 w-5" /> {showEntregas ? 'Ocultar notas' : 'Revisar notas'}</>
            ...
            {/* Summary Table */}
            <div className="mb-8">
              <h2 className="text-xl font-bold mb-4">{isQuiz ? 'Resumen del Cuestionario' : 'Resumen de la Actividad'}</h2>
              <div className="bg-card border border-border shadow-sm rounded-xl overflow-hidden text-sm">
                <table className="w-full text-left">
                  <tbody className="divide-y divide-border">
                    <tr className="divide-x divide-border">
                      <td className="p-4 w-1/3 font-semibold text-muted-foreground bg-muted/10">{isQuiz ? 'Intentos permitidos' : 'Fecha límite de entrega'}</td>
                      <td className="p-4 text-foreground">{isQuiz ? '1 intento' : 'Sin fecha definida'}</td>
                    </tr>
                    <tr className="divide-x divide-border">
                      <td className="p-4 font-semibold text-muted-foreground bg-muted/10">{isQuiz ? 'Intentos registrados' : 'Entregas realizadas'}</td>
                      <td className="p-4">
                        {loadingEntregas ? <span className="text-muted-foreground italic">Cargando…</span> : (
            ...
            {showEntregas && (
              <div className="mb-12 animate-in slide-in-from-top-4 duration-300">
                ...
                <div className="bg-card border border-border rounded-xl overflow-hidden shadow-sm">
                  {entregas.length === 0 ? (
                    <div className="p-12 text-center text-muted-foreground italic">No hay entregas aún para esta actividad.</div>
                  ) : (
                        ...
                        {entregas.map((e: any) => (
                          <tr key={e.guid} className="hover:bg-muted/20 transition-colors">
                            ...
                            <td className="p-4 text-muted-foreground text-xs">
                              {e.fecha_entrega ? new Date(e.fecha_entrega).toLocaleString('es-ES', { weekday: 'short', day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' }) : '—'}
                            </td>
                            <td className="p-4">
                              <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-bold ${e.estado === 'ENTREGADA' ? 'bg-emerald-500/10 text-emerald-600' : 'bg-amber-500/10 text-amber-600'}`}>
        ...
        <>
            ...
            {uploadState === 'done' && selectedFileName ? (
              ...
              <div className="flex flex-col items-center text-center gap-4 py-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Arrastra un archivo aquí o haz clic para seleccionarlo</p>
                </div>
                <button
                  onClick={() => fileInputRef.current?.click()}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="bg-card border border-border/50 rounded-xl p-5 flex items-start gap-4">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${uploadState === 'done' ? 'bg-emerald-500/10' : 'bg-muted'}`}>
                {uploadState === 'done' ? <CheckCircle2 className="h-5 w-5 text-emerald-500" /> : <Clock className="h-5 w-5 text-muted-foreground" />}
              </div>
              <div>
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Estado</p>
                <p className={`text-sm font-bold mt-0.5 ${uploadState === 'done' ? 'text-emerald-600' : 'text-foreground'}`}>
            ...
            {/* Calificación status */}
            <div className="bg-card border border-border/50 rounded-xl p-5 flex items-start gap-4">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${calificacion ? 'bg-amber-500/10' : 'bg-muted'}`}>
            ...
            <div className="bg-card border border-border/50 rounded-xl p-5 flex items-start gap-4 sm:col-span-2">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${uploadState === 'done' ? 'bg-primary/10' : 'bg-muted'}`}>
                <FileIcon className={`h-5 w-5 ${uploadState === 'done' ? 'text-primary' : 'text-muted-foreground'}`} />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Archivo entregado</p>
                {uploadState === 'done' && selectedFileName ? (
